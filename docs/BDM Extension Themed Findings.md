...
> [!info] Mike's Use Case
> After a call with Mike on 11 Sept, we agreed that he wouldn't have much use for Vale; most of his outreach is to warm clients, rather than cold outreach (a la David and Muki), and is sending single emails rather than bulk at a single time
> We agreed to park this and that we would cross this bridge when we get to it and he finds a use case for such a tool
...
- **There are four spreadsheets in the fleet, not three:** David, Muki, Mike, plus **Wesley's own Vale test sheet**, which is the build/break environment. The test sheet points at the newest library version (effectively head) and sends no real outreach. Confirmed-good changes get promoted from there to individual BDM pins.
	- [i] Mike's spreadsheet is still pending, given he doesn't have a strict case for it just yet
...
- **Mike is not blocking Muki.** Whatever his workflow turns out to need becomes local logic in his own container script when it arrives — it will not touch Muki's sheet or her library pin.
- Sequencing stands as originally decided: Muki's build first, Mike's build once he determines a use case on sufficient business volume
